...
/**
 * PEAR required for error handling
 */
require_once 'PEAR.php';
/**
 * File package required for file processing
 */
require_once 'File.php';

/**
* Package to handle CSV (Comma Separated Values) files
*
* CSV files are often used to import and export data between programs 
* (such as Microsoft Excel) or databases that otherwise use incompatible
* file formats.  A CSV file contains data from a spreadsheet or database 
* table that is organized so that each row or record corresponds to a line 
* in the CSV file.  Each cell is separated by a delimeter, often commas, 
* tabs, or vertical bars ("|").
*
* Example CSV file:
* <code>
...
* </code>
*
* In this simple example, the data in the first row is a "header,"
* which gives names to each column.  In each row, the data before the first
* comma corresponds to a customer's last name.  The second "cell" is
* the first name, and the third is a phone number.
*
* Known Bugs:
* (they have been analyzed but for the moment the impact in the speed to
*  properly handle these uncommon cases is too high and won't be supported)
...
*
* @todo Usage example and Doc
* @todo Use getPointer() in discoverFormat
* @todo Add a line counter for being able to output better error reports
* @todo Store the last error in GLOBALS and add File_CSV::getLastError()
* @todo Support Mac EOL format
* @todo Other methods like readAll(), writeAll(), numFields(), numRows()
* @todo Try to detect if a CSV has header or not in discoverFormat()
*
