...
function TableOperations(editor) {
	...
	if ( !_editor_tableflag )
	{
	    var a, i, j, found = false;
    	for (i = 0; !found && i < cfg.toolbar.length; ++i) {
    		a = cfg.toolbar[i];
    	...
    	if (found)
    	{
    	    ...
    	    {
    	        a.splice(j+i+1, 0, toolbar[i]);
    	    }
    	}
        else
        {                
    	    cfg.toolbar[0] = cfg.toolbar[0].concat(toolbar);
        }
	    _editor_tableflag = true;
	}
};
...
TableOperations.prototype.dialogTableProperties = function() {
	...
	var atts = '';
	if ( table.className != '' )
	    atts += 'class|'+table.className+'|';
	if ( table.width != '' )
	    atts += 'width|'+escape(table.width)+'|';
	if ( table.border != '' )
	    atts += 'border|'+table.border+'|';
	this.editor._popupDialog("xmlarea:/" + _editor_indexfile + "/xmlarea/tag/table/Table/" + atts,
        	function(param)
        	{
         		if (!param)
         	    ...
         	    for ( i in param['atts'] )
         	    {
         	        ...
         	        {
         	            table.className = param['atts'][i][1];
         	        }
         	        else if ( param['atts'][i][0] == 'width' )
         	        {
             	        table.width = param['atts'][i][1];
             	    }
         	        else if ( param['atts'][i][0] == 'border' )
         	        {
             	        table.border = param['atts'][i][1];
             	    }
...
TableOperations.prototype.dialogRowCellProperties = function(cell) {
    var elName = ( cell ) ? 'td' : 'th';
	var i18n = TableOperations.I18N;
	// retrieve existing values
	var element = this.getClosest(elName);
	//var table = this.getClosest("table");
		var atts = '';
	if ( element.className != '' )
	    atts += 'class|'+element.className+'|';
	if ( element.width != '' )
	    atts += 'width|'+escape(element.width)+'|';
	if ( element.colSpan != '' )
	    atts += 'colSpan|'+element.colSpan+'|';
	if ( element.rowSpan != '' )
	    atts += 'rowSpan|'+element.rowSpan+'|';
	this.editor._popupDialog("xmlarea:/" + _editor_indexfile + "/xmlarea/tag/" + elName + "/Cell/" + atts,
        	function(param)
        	{
         		if (!param)
         		{	// user must have pressed Cancel
         	    ...
         	    for ( i in param['atts'] )
         	    {
         	        if ( param['atts'][i][0] == 'class' )
         	        {
         	            element.className = param['atts'][i][1];
         	        }
         	        else if ( param['atts'][i][0] == 'width' )
         	        {
             	        element.width = param['atts'][i][1];
             	    }
        	    }
